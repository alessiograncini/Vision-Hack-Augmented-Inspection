...
public class OptimizedCubePointCloudVisualizer : MonoBehaviour
{
    ...
    private void InitializeCubePool()
    {
        ...
        propertyBlocks = new List<MaterialPropertyBlock>(maxCubes);
    ...
    private void ProcessPointCloudData(string jsonData)
    {
        ...
        if (data["status"].ToString() == "success")
        {
            ...
            for (int i = 0; i < pointCount; i++)
            {
                ...
                MaterialPropertyBlock props = propertyBlocks[i];
                props.SetColor("_BaseColor", new Color(colors[i][0], colors[i][1], colors[i][2]));
